...
public class JSONRPCServlet extends HttpServlet {
     ...
     * counter productive for 2 reasons:
     * 
     * 1. if the response is really small, the gzipped output can actually be
     * larger than the non-compressed original. because of the gzip header and
     * the general overhead of the gzipping. This is a lose-lose situation, so
     * the original should always be sent in this case.
     * 
     * 2. gzipping imposes a small performance penality in that it takes a
     * little more time to gzip the content. There is also a corresponding
     * penality on the browser side when the content has to be uncompressed.
     * 
     * This penalty is really small, and is normally more than outweighed by the
     ...
     * repetition.
     * 
     * So, the GZIP_THRESHOLD should be tuned to a size that is optimal for your
     * application. If your application is always served from a high speed
     * network, you might want to set this to a very high number-- (or even -1
     * to turn it off) for slower networks where it's more important to conserve
     * bandwidth, set this to a lower number (but not too low!)
     * 
     * Set this to zero if you want to always attempt to gzip the output when
     * the browser can accept gzip encoded responses. This is useful for
     * analyzing what a good gzip setting should be for potential responses from
     * your application.
     * 
     * You can set this to -1 if you want to turn off gzip encoding for some
     * reason.
     */
